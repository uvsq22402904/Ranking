...
int main() {
  ...
  if (N == 0 || M == 0 || p == NULL) {
    fprintf(stderr,
            "Erreur lors de la lecture et normalisation de la matrice.\n");
    return 1;
  }
  ...
  if (x == NULL || y == NULL) {
    fprintf(stderr,
            "Erreur d'allocation mémoire pour les vecteurs PageRank.\n");
    liberer_memoire();
    return 1;
  }
...
void lire_et_normaliser_mtx(char *nom_fichier) {
  ...
  if (sscanf(ligne, "%d %*d %d", &N, &L) != 2) {
    ...
    if (sscanf(ligne, "%d %d %d", &N, &N, &L) != 3) {
      fprintf(stderr,
              "Erreur: Impossible de lire les dimensions et le nombre "
              "d'éléments depuis l'en-tête.\n");
      fclose(f);
      exit(1);
    }
  }
  ...
  double val_elem;
  while (m_courant < M && fscanf(f, "%d %d %lf", &ligne_elem, &col_elem,
                                 &val_elem) == 3) {
    p[m_courant].i = ligne_elem - 1;
    p[m_courant].j = col_elem - 1;
    p[m_courant].val = 1.0;

    if (p[m_courant].i < 0 || p[m_courant].i >= N || p[m_courant].j < 0 ||
        p[m_courant].j >= N) {
      fprintf(stderr,
              "Attention: Index (%d, %d) hors limites [0, %d) lu à "
              "l'élément %d.\n",
  ...
  if (m_courant != M) {
    fprintf(stderr,
            "Attention: Nombre d'éléments lus (%d) différent de M annoncé "
            "(%d).\n",
            m_courant, M);
    M = m_courant;
...
void lire_et_normaliser_mtx_avec_suppression(char *nom_fichier) {
  ...
  while (elements_lus < L && fscanf(f, "%d %d %lf", &ligne_elem, &col_elem,
                                   &val_elem) == 3) {
    int i = ligne_elem - 1;
    int j = col_elem - 1;
...
void iteration_puissance() {
  ...
  while (diff > sigma) {
    ...
    if (isnan(diff) || isinf(diff)) {
      fprintf(stderr,
              "Erreur: Convergence échouée (NaN ou Inf détecté) à l'itération "
              "%d.\n",
              k);
      break;
    }
